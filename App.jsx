...
const CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  @media (max-width: 900px) {
    .gallery-grid { grid-template-columns: 1fr 1fr; grid-template-rows: 200px 200px 200px 160px; }
    .gc-big  { grid-column: 1 / 3 !important; grid-row: auto !important; }
    .gc-wide { grid-column: 1 / 3 !important; grid-row: auto !important; }
    .gc-sm   { grid-column: auto !important; grid-row: auto !important; }
  }
...
function GallerySection() {
  const cells = [
    { id: 0, label: "The Kitchen",    cls: "gc-big",  style: { gridColumn: "1/3", gridRow: "1/3" },
      render: () => <div style={{ position:"absolute", inset:0, background:"radial-gradient(ellipse at 38% 42%, rgba(110,60,15,.45) 0%, transparent 52%), radial-gradient(ellipse at 65% 65%, rgba(70,30,8,.4) 0%, transparent 48%), #0E0C08" }}>
    ...
    { id: 1, label: "Saffron Dusk",   cls: "gc-sm",   style: { gridColumn: "3", gridRow: "1" },
      render: () => <div style={{ position:"absolute", inset:0, background:"linear-gradient(148deg, #1E1208 0%, #0E0B08 55%, #0C0B08 100%)" }}>
    ...
    { id: 2, label: "Noir & Marble",  cls: "gc-sm",   style: { gridColumn: "3", gridRow: "2" },
      render: () => <div style={{ position:"absolute", inset:0, background:"linear-gradient(120deg, #1A1815 0%, #0C0B08 100%)" }}>
        {[[20, .06, "20deg"],[45, .04, "-6deg"],[70, .05, "12deg"]].map(([t,o,r],i)=>(
          <div key={i} style={{ position:"absolute", top:`${t}%`, left:"8%", right:"25%", height:1, background:`rgba(240,235,227,${o})`, transform:`rotate(${r})` }} />
        ))}
        <div style={{ position:"absolute", bottom:"20%", right:"15%", width:3, height:3, borderRadius:"50%", background:"var(--gold)", opacity:.7 }} />
      </div>
    },
    { id: 3, label: "Vineyard Dusk",  cls: "gc-sm",   style: { gridColumn: "1", gridRow: "3" },
      render: () => <div style={{ position:"absolute", inset:0, background:"linear-gradient(138deg, #140810 0%, #0C0508 100%)" }}>
    ...
    { id: 4, label: "The Terrace",    cls: "gc-wide", style: { gridColumn: "2/4", gridRow: "3" },
      render: () => <div style={{ position:"absolute", inset:0, background:"linear-gradient(to bottom, #0A0C12, #0C0B08)" }}>
        {[[8,12],[18,8],[32,18],[45,6],[55,20],[65,10],[78,15],[88,8],[93,22],[25,30],[42,14],[70,28]].map(([l,t],i)=>(
          <div key={i} style={{ position:"absolute", left:`${l}%`, top:`${t}%`, width:i%3===0?2:1.5, height:i%3===0?2:1.5, borderRadius:"50%", background:`rgba(255,255,255,${.28+i*.04})` }} />
        ))}
          ...
          <div className="gallery-grid">
            {cells.map(cell => (
              <div key={cell.id} className={`gc ${cell.cls}`} style={cell.style}>
                <div className="art" style={{ position: "relative", width: "100%", height: "100%" }}>{cell.render()}</div>
                <div className="ov"><span className="fs" style={{ fontSize: 11, letterSpacing: "0.2em", textTransform: "uppercase", color: "var(--gold)" }}>{cell.label}</span></div>
              </div>
            ))}
          </div>
        </R>
  ...
  return (
    <div style={{ background: "var(--card)", borderTop: "1px solid rgba(196,162,89,.1)", borderBottom: "1px solid rgba(196,162,89,.1)", padding: "17px 0", overflow: "hidden" }}>
      <div className="marquee-track">
        {doubled.map((a, i) => (
          <span key={i} className="fs" style={{ padding: "0 52px", fontSize: 11, letterSpacing: "0.18em", textTransform: "uppercase", color: "var(--muted)", whiteSpace: "nowrap" }}>{a}</span>
        ))}
      </div>
    </div>
  );
        ...
        <div className="footer-grid" style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr", gap: 56, marginBottom: 72, paddingBottom: 60, borderBottom: "1px solid var(--border)" }}>
          <div>
            ...
            <div style={{ display: "flex", gap: 12 }}>
              {["IG","FB","TW","YT"].map(s => (
                <div key={s} style={{ width: 34, height: 34, border: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
                  <span className="fs" style={{ fontSize: 10, color: "var(--muted)" }}>{s}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <div style={{ fontSize: 15, color: "var(--muted)", lineHeight: 1.85 }}>
              ...
              <div>Apollo Bunder, Mumbai 400 001</div>
